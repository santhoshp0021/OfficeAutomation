  ...
  const handleSubmitAssignments = async () => {
    ...
    try {
      ...
      for (const a of assignments) {
        if (!a.slot && attendanceStatus[a.teamId]) throw new Error('All eligible teams must be assigned a slot.');
        if (!attendanceStatus[a.teamId]) continue;
      ...
      await axios.post(
        ...
        {
          slotType: 'viva',
          assignments: assignments.filter(a => attendanceStatus[a.teamId]).map((a) => ({
            teamId: a.teamId,
            slot: a.slot,
          })),
        },
          ...
          <table className="w-full border mb-4">
            ...
            <tbody>
              {teams.map((team, idx) => {
                ...
                const scheduleExists = scheduleStatus[team._id];
                return (
                  <tr key={team._id}>
          ...
          <div className="space-y-4">
            {allottedSchedules.map(schedule => (
              <div key={schedule._id} className="border rounded-lg p-4 bg-white">
                <h4 className="text-lg font-semibold mb-2">Viva</h4>
