...
export default function ConsolidationReportFacultyAnalytics() {
  ...
  if (loading || loadingCR)
    return <div className="p-10 text-center">Loading Faculty Analytics...</div>;

  // Find the selected faculty by name (case-insensitive)
  const selectedFaculty = faculty.find(
    (f) =>
      f.name &&
      f.name.toLowerCase() === decodeURIComponent(facultyName).toLowerCase()
  );
  ...
  const completedScholars = supervisedScholars.filter(
    (s) => s.dateOfCompletion && new Date(s.dateOfCompletion) < new Date()
  );
  const presentScholars = supervisedScholars.filter(
    (s) => !s.dateOfCompletion || new Date(s.dateOfCompletion) >= new Date()
  );
  ...
  const eventsAttended = facultyOD.filter(
    (ev) =>
      ev.endDate &&
      new Date(ev.endDate) < today &&
      (ev.status || "").toLowerCase() === "approved"
  );
  const ongoingEvents = facultyOD.filter(
    (ev) =>
      ev.startDate &&
      ev.endDate &&
      new Date(ev.startDate) <= today &&
      today <= new Date(ev.endDate) &&
      (ev.status || "").toLowerCase() === "approved"
  );
  const upcomingEvents = facultyOD.filter(
    (ev) => ev.startDate && new Date(ev.startDate) > today
  );
  ...
  const handleExportPDF = async () => {
    ...
    await pdf.html(content, {
      ...
      callback: function (doc) {
        ...
        doc.save(
          `faculty_analytics_${selectedFaculty.name.replace(/\s+/g, "_")}${
            selectedSemester ? `_semester_${selectedSemester}` : ""
          }.pdf`
        );
      },
    });
  ...
  if (isExportingPDF) {
    ...
    const completionDateHeaderStyle = {
      ...lastColStyle,
      fontWeight: 700,
      minWidth: "140px",
      fontSize: "7px",
    };
    const completionDateCellStyle = {
      ...lastColStyle,
      minWidth: "140px",
      fontSize: "7px",
    };
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div style={{ fontWeight: 700 }}>
            Faculty Consolidation Report
            {selectedSemester ? ` - Semester ${selectedSemester}` : ""}
          </div>
          <div
            style={{ borderTop: "0.5px solid #333333", margin: "8px 0 10px 0" }}
          />
        </div>
        {/* Basic Info */}
        <div style={{ marginBottom: 0.2 }}>
          ...
          >
            <tbody>
              ...
              <tr>
                <td style={{ ...cellStyle, fontWeight: 600, width: 120 }}>
                  Department:
                </td>
                <td
                  style={{ ...lastColStyle, borderLeft: "0.5px solid #333333" }}
                >
              ...
              <tr>
                ...
                <td
                  style={{ ...lastColStyle, borderLeft: "0.5px solid #333333" }}
                >
                  {selectedFaculty.contactInfo?.email || "-"}
                </td>
              </tr>
              <tr>
                <td style={{ ...cellStyle, fontWeight: 600, width: 120 }}>
                ...
                <td
                  style={{ ...lastColStyle, borderLeft: "0.5px solid #333333" }}
                >
                  {selectedFaculty.contactInfo?.phone || "-"}
                </td>
              </tr>
              <tr>
                <td style={{ ...cellStyle, fontWeight: 600, width: 120 }}>
              ...
              <tr>
                ...
                <td
                  style={{ ...lastColStyle, borderLeft: "0.5px solid #333333" }}
                >
                  {selectedFaculty.isActive ? "Yes" : "No"}
                </td>
              </tr>
              <tr>
                <td style={{ ...cellStyle, fontWeight: 600, width: 120 }}>
        ...
        <div style={{ marginBottom: 0.2 }}>
          <div
            style={{ fontWeight: 700, margin: "8px 0 2px 0", fontSize: "10px" }}
          >
            Scholars Supervised ({supervisedScholars.length}):
          </div>
          <div style={{ marginBottom: 8 }}>
            Total Scholars: {supervisedScholars.length}
          </div>
            ...
            >
              ...
              <tbody>
                {supervisedScholars.map((s) => {
                  const isCompleted =
                    s.dateOfCompletion &&
                    new Date(s.dateOfCompletion) < new Date();
                  return (
                    <tr key={s._id}>
                      <td
                        style={{ ...cellStyle, border: "0.5px solid #333333" }}
                      >
                        {s.name || "-"}
                      </td>
                      <td
        ...
        <div style={{ marginBottom: 0.2 }}>
          <div
            style={{ fontWeight: 700, margin: "8px 0 2px 0", fontSize: "10px" }}
          >
            Event History (OD Requests)
          </div>
          <div>Total Events: {facultyOD.length}</div>
          {/* Events Attended */}
          <div
            style={{ fontWeight: 700, margin: "8px 0 16px 0", fontSize: "9px" }}
          >
            ...
            >
              ...
              <tbody>
                {eventsAttended.map((ev) => (
                  <tr key={ev._id}>
                    <td style={{ ...cellStyle, border: "0.5px solid #333333" }}>
                      {ev.topic || "-"}
                    </td>
                    <td style={{ ...cellStyle, border: "0.5px solid #333333" }}>
                      {ev.eventType || ev.requestType || "-"}
                    </td>
                    <td style={{ ...cellStyle, border: "0.5px solid #333333" }}>
                      {ev.startDate
  ...
  return (
    <div className="p-4 md:p-10 space-y-10">
      ...
      <div
        id="faculty-analytics-content"
        className="pdf-export pdf-sample-font pdf-export-content"
      >
        ...
        <section>
          ...
          <table className="pdf-basic-info-table">
            <tbody>
              <tr>
                <td className="pdf-basic-info-label">Name:</td>
                <td>{selectedFaculty.name || "-"}</td>
              </tr>
              <tr>
                <td className="pdf-basic-info-label">Department:</td>
                <td>{selectedFaculty.department || "-"}</td>
              </tr>
              <tr>
                <td className="pdf-basic-info-label">Designation:</td>
                <td>{selectedFaculty.designation || "-"}</td>
              </tr>
              <tr>
                <td className="pdf-basic-info-label">Email:</td>
                <td>{selectedFaculty.contactInfo?.email || "-"}</td>
              </tr>
              <tr>
                <td className="pdf-basic-info-label">Phone:</td>
                <td>{selectedFaculty.contactInfo?.phone || "-"}</td>
              </tr>
              <tr>
                <td className="pdf-basic-info-label">Gender:</td>
                <td>{selectedFaculty.gender || "-"}</td>
              </tr>
              <tr>
                <td className="pdf-basic-info-label">Date of Birth:</td>
                <td>
                  {selectedFaculty.dob
                    ? new Date(selectedFaculty.dob).toLocaleDateString("en-IN")
                    : "-"}
              ...
              <tr>
                <td className="pdf-basic-info-label">Date of Joining:</td>
                <td>
                  {selectedFaculty.dateOfJoining
                    ? new Date(
                        selectedFaculty.dateOfJoining
              ...
              <tr>
                <td className="pdf-basic-info-label">Active:</td>
                <td>{selectedFaculty.isActive ? "Yes" : "No"}</td>
              </tr>
              <tr>
                <td className="pdf-basic-info-label">Years of Experience:</td>
                <td>
                  {selectedFaculty.dateOfJoining
                    ? new Date().getFullYear() -
                      new Date(selectedFaculty.dateOfJoining).getFullYear()
              ...
              <tr>
                <td className="pdf-basic-info-label">Areas of Expertise:</td>
                <td>{selectedFaculty.areasOfExpertise?.join(", ") || "-"}</td>
              </tr>
            </tbody>
          </table>
        </section>
        ...
        <section>
          ...
          <div>Total Events: {facultyOD.length}</div>
          {/* Events Attended */}
          <div className="pdf-section-subheading">
            Events Attended ({eventsAttended.length}):
          </div>
          {eventsAttended.length > 0 ? (
            <table>
