...
function getYears(arr, dateKey) {
  return Array.from(
    new Set(
      arr
        .map((item) =>
          item[dateKey] ? new Date(item[dateKey]).getFullYear() : null
        )
        .filter(Boolean)
    )
  );
    ...
    <div className="flex flex-col min-h-[70vh] justify-between items-center p-10">
      <div className="flex-1 flex flex-col justify-center items-center w-full">
        <h1 className="text-3xl font-bold mb-8 text-center">
          Select and view the consolidation reports
        </h1>
      </div>
...
export function ConsolidationReportScholars() {
  ...
  const filteredScholars = scholars.filter((s) => {
    ...
    if (scholarSupervisor && s.supervisor?.name !== scholarSupervisor)
      ok = false;
    if (scholarStatus) {
      const isCurrent =
        !s.dateOfCompletion || new Date(s.dateOfCompletion) >= new Date();
    ...
    if (
      scholarYear &&
      s.dateOfJoining &&
      new Date(s.dateOfJoining).getFullYear().toString() !== scholarYear
    )
      ok = false;
  ...
  const filteredOD = odRequests.filter((r) => {
    ...
    if (
      odYear &&
      r.date &&
      new Date(r.date).getFullYear().toString() !== odYear
    )
      ok = false;
  ...
  const filteredPubs = publications.filter((p) => {
    ...
    if (
      pubFaculty &&
      (Array.isArray(p.authors) ? p.authors[0] : p.authors) !== pubFaculty
    )
      ok = false;
    if (
      pubYear &&
  ...
  // Scholar breakdowns
  const scholarsByArea = useMemo(
    () => groupBy(filteredScholars, (s) => s.areaOfResearch || "Unknown"),
    [filteredScholars]
  );
  const scholarsBySupervisor = useMemo(
    () => groupBy(filteredScholars, (s) => s.supervisor?.name || "Unknown"),
    [filteredScholars]
  );
  const scholarsByDept = useMemo(
    () => groupBy(filteredScholars, (s) => s.department || "Unknown"),
    [filteredScholars]
  );
  const scholarsByJoiningYear = useMemo(
    () =>
      groupBy(filteredScholars, (s) =>
        s.dateOfJoining ? new Date(s.dateOfJoining).getFullYear() : "Unknown"
      ),
    [filteredScholars]
  );
  const scholarsByCompletionYear = useMemo(
    () =>
      groupBy(
        filteredScholars.filter((s) => s.dateOfCompletion),
        (s) => new Date(s.dateOfCompletion).getFullYear()
      ),
    [filteredScholars]
  );
  const scholarStatusCounts = useMemo(() => {
    ...
    filteredScholars.forEach((s) => {
      if (s.dateOfCompletion && new Date(s.dateOfCompletion) < new Date())
        status.Completed++;
      else status.Current++;
    });
    return status;
  ...
  const currentScholars = filteredScholars.filter(
    (s) => !s.dateOfCompletion || new Date(s.dateOfCompletion) >= new Date()
  ).length;
  const completedScholars = filteredScholars.filter(
    (s) => s.dateOfCompletion && new Date(s.dateOfCompletion) < new Date()
  ).length;
  ...
  const pubsByFaculty = groupBy(
    filteredPubs,
    (p) => (Array.isArray(p.authors) ? p.authors[0] : p.authors) || "Unknown"
  );
  ...
  const facultyByDept = groupBy(
    filteredFaculty,
    (f) => f.department || "Unknown"
  );
  const facultyByDesignation = groupBy(
    filteredFaculty,
    (f) => f.designation || "Unknown"
  );
      ...
      <div className="fixed inset-0 bg-black bg-opacity-40 z-[2000] flex items-center justify-center">
        <div className="bg-white rounded shadow-lg p-6 max-h-[80vh] w-[90vw] overflow-auto">
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              onClick={() => setDrilldown({ open: false, title: "", data: [] })}
              className="text-2xl"
            >
              &times;
            </button>
  ...
  // Scholar lists
  const currentScholarsList = filteredScholars.filter(
    (s) => !s.dateOfCompletion || new Date(s.dateOfCompletion) >= new Date()
  );
  const completedScholarsList = filteredScholars.filter(
    (s) => s.dateOfCompletion && new Date(s.dateOfCompletion) < new Date()
  );
  ...
  function ScholarStatusPie() {
    ...
    const completedPct = total
      ? (scholarStatusCounts.Completed / total) * 100
      : 0;
    ...
    const d = `M${c},${
      cy - r
    } A${r},${r} 0 ${largeArc} 1 ${x},${y} L${c},${cy} Z`;
    return (
      <div className="flex flex-col items-center">
        ...
        <div className="flex gap-4 mt-2 text-xs">
          <div className="flex items-center gap-1">
            ...
            Current ({scholarStatusCounts.Current}, {currentPct.toFixed(1)}%)
          </div>
          <div className="flex items-center gap-1">
            ...
            <div key={k} className="flex items-center gap-2">
              ...
              <div className="flex-1 h-4 bg-gray-100 rounded relative">
                <div
                  ...
                  style={{ width: `${(v / maxVal) * 100}%`, background: color }}
                />
                <span className="absolute left-2 top-0 text-xs text-white font-bold">
                  {v}
                </span>
              </div>
            </div>
  ...
  function ScholarsStackedBarChart() {
    ...
    const maxVal = Math.max(
      ...years.map(
        (y) =>
          (scholarsByJoiningYear[y] || 0) + (scholarsByCompletionYear[y] || 0)
      ),
      1
    );
    return (
        ...
        {years.map((y) => (
          <div key={y} className="flex items-center gap-2 mb-1">
            ...
            <div className="flex-1 h-4 bg-blue-100 rounded relative">
              <div
                className="absolute left-0 top-0 h-4 bg-blue-500 rounded-l"
                style={{
                  width: `${((scholarsByJoiningYear[y] || 0) / maxVal) * 100}%`,
                }}
              />
              <span className="absolute left-2 top-0 text-xs text-white">
                {scholarsByJoiningYear[y] || 0}
              </span>
            </div>
            <div className="flex-1 h-4 bg-green-100 rounded relative">
              <div
                className="absolute left-0 top-0 h-4 bg-green-500 rounded-l"
                style={{
                  width: `${
                    ((scholarsByCompletionYear[y] || 0) / maxVal) * 100
                  }%`,
                }}
              />
              <span className="absolute left-2 top-0 text-xs text-white">
                {scholarsByCompletionYear[y] || 0}
              </span>
            </div>
          </div>
        ))}
  ...
  // Advanced Analytics
  // 1. Average Duration
  const completedScholarsForDuration = filteredScholars.filter(
    (s) => s.dateOfJoining && s.dateOfCompletion
  );
  const avgDurationMonths =
    completedScholarsForDuration.length > 0
  ...
  const supervisorLoadEntries = Object.entries(supervisorLoads).sort(
    (a, b) => b[1] - a[1]
  );
  ...
  const trendsCompleted = trendsYears.map(
    (y) => scholarsByCompletionYear[y] || 0
  );
          ...
          {entries.map(([k, v], i) => (
            <div key={k} className="flex items-center gap-1">
              <span
                ...
                style={{
                  background: colors[i % colors.length].replace(
                    "oklch",
                    "#888"
                  ),
                }}
        ...
        {trendsYears.map((y, i) => (
          <div key={y} className="flex items-center gap-2 mb-1">
            ...
            <div className="flex-1 h-4 bg-blue-100 rounded relative">
              <div
                className="absolute left-0 top-0 h-4 bg-blue-500 rounded-l"
                style={{ width: `${(trendsJoined[i] / maxVal) * 100}%` }}
              />
              <span className="absolute left-2 top-0 text-xs text-white">
                {trendsJoined[i]}
              </span>
            ...
            <div className="flex-1 h-4 bg-green-100 rounded relative">
              <div
                className="absolute left-0 top-0 h-4 bg-green-500 rounded-l"
                style={{ width: `${(trendsCompleted[i] / maxVal) * 100}%` }}
              />
              <span className="absolute left-2 top-0 text-xs text-white">
                {trendsCompleted[i]}
              </span>
  ...
  async function handleExportPDF() {
    try {
      ...
      if (err && err.message && err.message.includes("oklch")) {
        if (
          window.confirm(
            "PDF export failed due to unsupported color (oklch). Would you like to export a text-based PDF instead?"
          )
        ) {
          exportTextPDF();
        }
  ...
  function exportTextPDF() {
    ...
    autoTable(pdf, {
      startY: y,
      head: [["Area", "Count"]],
      body: Object.entries(scholarsByArea).map(([area, count]) => [
        area,
        count,
      ]),
      theme: "grid",
    ...
    autoTable(pdf, {
      ...
      body: Object.entries(scholarsBySupervisor).map(([sup, count]) => [
        sup,
        count,
      ]),
      theme: "grid",
    ...
    autoTable(pdf, {
      ...
      body: Array.from(
        ...
        .map((year) => [
          year,
          scholarsByJoiningYear[year] || 0,
          scholarsByCompletionYear[year] || 0,
        ]),
      theme: "grid",
      styles: { fontSize: 10 },
      margin: { left: 40, right: 40 },
    ...
    pdf.setFont(undefined, "normal");
    pdf.text(
      `Average Duration to Completion: ${avgDurationYears}y ${avgDurationRemMonths}m`,
      40,
      y
    );
    y += 16;
  ...
  odRequests.forEach((r) => {
    ...
    if (!facultyTypeStatus[r.name][r.requestType])
      facultyTypeStatus[r.name][r.requestType] = {};
    facultyTypeStatus[r.name][r.requestType][r.status] =
      (facultyTypeStatus[r.name][r.requestType][r.status] || 0) + 1;
  });
  const allTypes = Array.from(
    new Set(odRequests.map((r) => r.requestType))
  ).filter((t) => t && t !== "Conduct" && t !== "Participate");
  const allStatuses = Array.from(new Set(odRequests.map((r) => r.status)));

  return (
    <div className="p-4 md:p-10 space-y-10">
      {/* Hidden test div for PDF troubleshooting */}
      <div id="pdf-test-div" style={{ display: "none" }}>
        PDF Export Test: If you see this in your PDF, export is working.
      </div>
      ...
      <div id="consolidation-report-export">
        ...
        <section className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 mb-10">
            ...
            <div className="bg-white rounded-xl shadow p-5 flex flex-col mb-8">
              ...
              <HorizontalBarChart
                data={scholarsBySupervisor}
                color="#f59e42"
                label="Supervisor"
                showAll={showAllSupervisors}
                setShowAll={setShowAllSupervisors}
              />
            </div>
              ...
              <table className="text-sm w-full mt-2">
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="px-2 py-1">{scholarStatusCounts.Current}</td>
                  </tr>
                  <tr>
                    <td className="px-2 py-1">Completed</td>
                    <td className="px-2 py-1">
                      {scholarStatusCounts.Completed}
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 mb-10">
            ...
            <div className="bg-white rounded-xl shadow p-5 flex flex-col items-center justify-center mb-8">
              ...
              <div className="text-4xl font-bold text-blue-700">
                {avgDurationYears}y {avgDurationRemMonths}m
              </div>
              <div className="text-xs text-gray-500 mt-1">
                (From joining to completion, among completed scholars)
              </div>
            </div>
            {/* Supervisor Load */}
            <div className="bg-white rounded-xl shadow p-5 flex flex-col">
              ...
              <div className="space-y-1">
                {supervisorLoadEntries.map(([sup, count]) => (
                  <div key={sup} className="flex items-center gap-2">
                    ...
                    {count === minLoad && (
                      <span className="text-red-700 text-xs font-bold ml-2">
                        Low
                      </span>
                    )}
                  </div>
              ...
              <div className="text-xs text-gray-500 mt-1">
                (Number of scholars per supervisor)
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-xl shadow p-5 flex flex-col items-center col-span-1 md:col-span-2 xl:col-span-2">
              <h3 className="font-semibold mb-2 text-lg">
                Scholars Joined/Completed per Year
              </h3>
              <ScholarsTrendsChart />
            </div>
            ...
            <div className="bg-white rounded-xl shadow p-5 flex flex-col items-center">
              ...
              <PieChart
                data={scholarStatusCounts}
                colors={["#60a5fa", "#a3e635"]}
                label="Status"
              />
            </div>
          </div>
        ...
        <section>
          <h2 className="text-2xl font-semibold mb-2">
            Scholars Nearing Completion (Next 6 Months)
          </h2>
          <div className="overflow-x-auto">
            <table className="min-w-full bg-white rounded shadow text-sm">
              ...
              <tbody>
                {nearingCompletionList.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center py-4">
                      No scholars nearing completion.
                    </td>
                  </tr>
                ) : (
...
export function ConsolidationReportOD() {
  ...
  const odByMonthData = Object.entries(odByMonth).map(([month, count]) => ({
    month,
    count,
  }));
  const approvalRateByMonthData = Object.entries(approvalByMonth).map(
    ([month, obj]) => ({
      month,
      rate: obj.total ? Math.round((obj.approved / obj.total) * 100) : 0,
    })
  );

  // 2. Faculty Insights
  const facultyCounts = Object.entries(odByFaculty).map(([faculty, count]) => ({
    faculty,
    count,
  }));
  ...
  odRequests.forEach((r) => {
    if (!facultyApproval[r.name])
      facultyApproval[r.name] = { total: 0, approved: 0 };
    facultyApproval[r.name].total++;
    if (r.status === "Approved") facultyApproval[r.name].approved++;
  });
  const facultyApprovalData = Object.entries(facultyApproval).map(
    ([faculty, obj]) => ({
      faculty,
      rate: obj.total ? Math.round((obj.approved / obj.total) * 100) : 0,
      total: obj.total,
  ...
  // 3. Type Analysis
  const typeCounts = Object.entries(odByType).map(([type, count]) => ({
    type,
    count,
  }));
  // Remove 'Conduct' and 'Participate' from typeCounts
  const filteredTypeCounts = typeCounts.filter(
    (item) => item.type !== "Conduct" && item.type !== "Participate"
  );
  typeCounts.sort((a, b) => b.count - a.count);
  // Approval rate by type
  const typeApproval = {};
  odRequests.forEach((r) => {
    if (!typeApproval[r.requestType])
      typeApproval[r.requestType] = { total: 0, approved: 0 };
    typeApproval[r.requestType].total++;
    if (r.status === "Approved") typeApproval[r.requestType].approved++;
  });
  let typeApprovalData = Object.entries(typeApproval).map(([type, obj]) => ({
    type,
    rate: obj.total ? Math.round((obj.approved / obj.total) * 100) : 0,
    total: obj.total,
  }));
  typeApprovalData = typeApprovalData.filter(
    (item) => item.type !== "Conduct" && item.type !== "Participate"
  );
  ...
  const durations = odRequests
    .map((r) => getDurationDays(r.startDate, r.endDate))
    .filter((d) => d !== null);
  ...
  async function handleExportODPDF() {
    try {
      const reportElement = document.getElementById(
        "od-consolidation-report-export"
      );
      if (!reportElement) throw new Error("Report element not found");
      const canvas = await html2canvas(reportElement, {
        scale: 2,
      ...
      pdf.setFontSize(22);
      pdf.setFont(undefined, "bold");
      pdf.text("OD Consolidation Report", pageWidth / 2, 40, {
        align: "center",
      });
  ...
  function exportODTextPDF() {
    ...
    autoTable(pdf, {
      startY: y,
      head: [["Faculty", "Count"]],
      body: Object.entries(odByFaculty).map(([faculty, count]) => [
        faculty,
        count,
      ]),
      theme: "grid",
    ...
    const facultyTypeStatusBody = Object.entries(facultyTypeStatus).map(
      ([faculty, typeObj]) => {
        const row = [faculty];
        allTypes.forEach((type) => {
          allStatuses.forEach((status) => {
    ...
    autoTable(pdf, {
      ...
      body: odRequests.map((r) => [
        ...
        r.startDate && r.endDate
          ? `${new Date(r.startDate).toLocaleDateString("en-IN")} - ${new Date(
              r.endDate
    ...
    <div className="p-4 md:p-10 space-y-10">
      ...
      <div id="od-consolidation-report-export">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-xl shadow p-5 flex flex-col">
            <h2 className="font-semibold mb-2 text-lg">
              Approval Rate by OD Type (%)
            </h2>
            <ResponsiveContainer width="100%" height={250}>
              <BarChart
                data={typeApprovalData}
                margin={{ top: 10, right: 10, left: 0, bottom: 0 }}
              >
                <XAxis dataKey="type" />
                <YAxis domain={[0, 100]} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-xl shadow p-5 flex flex-col">
            <h2 className="font-semibold mb-2 text-lg">
              Longest/Shortest OD Requests
            </h2>
            <ul className="text-sm">
              <li className="mb-2">
                <span className="font-semibold">Longest:</span>{" "}
                {longest
                  ? `${longest.name} (${longest.requestType}) - ${longest.duration} days (${longest.startDate} to ${longest.endDate})`
                  : "-"}
              </li>
              <li>
                <span className="font-semibold">Shortest:</span>{" "}
                {shortest
                  ? `${shortest.name} (${shortest.requestType}) - ${shortest.duration} days (${shortest.startDate} to ${shortest.endDate})`
                  : "-"}
              </li>
            </ul>
          </div>
        ...
        <div className="bg-white rounded-xl shadow p-5 mt-8">
          <h2 className="font-semibold mb-2 text-lg">
            OD Request Counts by Faculty, Type, and Status
          </h2>
          <div className="overflow-x-auto">
                ...
                <tr>
                  ...
                  {allTypes.map((type) =>
                    allStatuses.map((status) => (
                      <th key={type + status} className="px-2 py-1 border">
                        {type} ({status})
                      </th>
                    ))
                  )}
          ...
          <div className="bg-white rounded-xl shadow p-5 flex flex-col">
            ...
            <ul className="text-sm">
              <li className="flex justify-between border-b py-1">
                <span>Pending</span>
                <span className="font-bold">{pendingOD}</span>
              </li>
              <li className="flex justify-between border-b py-1">
                <span>Approved</span>
                <span className="font-bold">{approvedOD}</span>
              </li>
              <li className="flex justify-between border-b py-1">
                <span>Rejected</span>
                <span className="font-bold">{rejectedOD}</span>
              </li>
            </ul>
          </div>
        </div>
